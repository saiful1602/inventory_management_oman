...
<html lang="en" dir="ltr">
  <head>
    <meta charset="UTF-8">
    <title>Monthly Profit Cost Update</title>
    <!-- Boxiocns CDN Link -->
...
<body>
    ...
    <?php
        ...
        $query="SELECT * FROM `monthly_profit_cost` WHERE `invoice_no`='$invoice'";
        $q=mysqli_query($con, $query);
        if(mysqli_num_rows($q) > 0){
	?>
     <div style="padding-top:1rem;" class="container col-lg-10 rounded-3 text-center" style="background: white;">
      <label class="text-center fs-2 mb-3 headline2">Monthly Profit Cost Update</label>
        ...
        <table id="table" class="table table-striped table-bordered">
          ...
          <tbody>
            <?php
              ...
              $sel="SELECT `invoice_no`,`cost_name`,`reason`,TRUNCATE(`cost_amount`,3) AS amount,`date`
                    FROM `monthly_profit_cost` WHERE `invoice_no`='$invoice'";
  ...
  <script>
    ...
    function clearForm(){
      ...
      var costName=currentRow.find("td:eq(3)").text();
      var txtReason=currentRow.find("td:eq(4)").text();  
      var costAmount=currentRow.find("td:eq(5)").text(); 
      var datepicker=currentRow.find("td:eq(6)").text(); 
      ...
      $('#costName option[value="'+costName+'"]').attr('selected',true);
    ...
    $(document).on('click','#updateBtn',function(){
      ...
      if(datepicker !="" && txtInvoice !="" && txtReason !="" && costName !="" && costAmount !=""){
        ...
        $.ajax({
          ...
          success:function(response){
            swal({
                ...
                window.location ="mpcAdmin.php?invoice="+txtInvoice;
