...
<html lang="en" dir="ltr">
  <head>
    <meta charset="UTF-8">
    <title>Saqar Payment Update</title>
    <!-- Boxiocns CDN Link -->
...
<body>
    ...
    <?php
        ...
        $query="SELECT * FROM `saqar_payment` WHERE `invoice_no`='$invoice'";
        $q=mysqli_query($con, $query);
        if(mysqli_num_rows($q) > 0){
	?>
     <div style="padding-top:1rem;" class="container col-lg-10 rounded-3 text-center" style="background: white;">
      <label class="text-center fs-2 mb-3 headline2">Saqar Payment Update</label>
        ...
        <table id="table" class="table table-striped table-bordered">
          ...
          <tbody>
            <?php
              ...
              $sel="SELECT `invoice_no`,`carry_via`,TRUNCATE(`amount`,3) AS amount,`date`
                    FROM `saqar_payment` WHERE `invoice_no`='$invoice'";
    ...
    <div id="tableDiv" class="container col-lg-10 rounded-3 text-center" style="background: #4a91bd;padding-top:2rem; ">
      ...
      <div class="row mx-auto text-center mb-3">
        ...
        <div class="mb-3 col-lg-6 col-md-6 col-sm-12">
          ...
          <input type="text" class="col-lg-8 col-md-10 col-sm-12 form-control" id="carryVia" placeholder=" " maxlength="50" >
        </div>
  ...
  <script>
    ...
    function clearForm(){
      ...
      var carryVia=currentRow.find("td:eq(2)").text(); 
      var txtAmount=currentRow.find("td:eq(3)").text(); 
    ...
    $(document).on('click','#updateBtn',function(){
      ...
      if(datepicker !="" && txtInvoice !="" && carryVia !="" && txtAmount !=""){

        $.ajax({
          url: 'saqarPayUpdate.php',
          type: "POST",
          data:{
          ...
          success:function(response){
            swal({
                ...
                window.location ="saqarPayAdmin.php?invoice="+txtInvoice;
