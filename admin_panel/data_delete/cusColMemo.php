...
<body>
	...
	<div class="dateTimeOpr">
		<p class="title5" style="width: 40%;">Customer name :
		 <?php 
		 	$query="SELECT `customer_name` FROM `customer_collection` WHERE `invoice_no`='$invoice' GROUP BY `customer_name`";
		 	$q=mysqli_query($con, $query);
		 	if($q){
		...
		<p class="title5" style="width: 30%;">Address :
			<?php 
		 	$query="SELECT `customer_address` FROM `customer_collection` WHERE `invoice_no`='$invoice' GROUP BY `customer_address`";
		 	$q=mysqli_query($con, $query);
		 	if($q){
		 		$data=mysqli_fetch_assoc($q);
	...
	<table border="1px solid" style="width: 90%;margin-left: 5%;" class="data-table-total">
		...
		<!-- Select From database -->
				...
				$sel="SELECT `reason`,`amount`,`profit` FROM `customer_collection` WHERE `invoice_no`='$invoice'";
				...
				<tr>
					...
					<td align="center"><?php echo number_format($d,3); ?></td>
				</tr>
				<?php
				$count++;
				}
	...
	<input type="text" id="numWord" value="<?php echo number_format($c,3); ?>" hidden>
	</div>
