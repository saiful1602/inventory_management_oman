...
<head>
	...
	<title>Supplier Payment Memo</title>
	<style type="text/css">
		*{
...
<body>
	...
	<div class="dateTimeOpr">
		<p class="title5" style="width: 40%;">Customer name :
		 <?php 
		 	$query="SELECT `spName` FROM `supplier_payment` WHERE `invoice_no`='$invoice' GROUP BY `spName`";
		 	$q=mysqli_query($con, $query);
		 	if($q){
		...
		<p class="title5" style="width: 30%;">Address :
			<?php 
		 	$query="SELECT `spAddress` FROM `supplier_payment` WHERE `invoice_no`='$invoice' GROUP BY `spAddress`";
		 	$q=mysqli_query($con, $query);
		 	if($q){
		 		$data=mysqli_fetch_assoc($q);
		 		$Saddress=$data['spAddress'];
		 	}
		 	echo $Saddress;
		  ?>
		</p>
	...
	<table border="1px solid" style="width: 90%;margin-left: 5%;" class="data-table-total">
		...
		<!-- Select From database -->
				...
				$sel="SELECT `reason`,`spAmount` FROM `supplier_payment` WHERE `invoice_no`='$invoice'";
