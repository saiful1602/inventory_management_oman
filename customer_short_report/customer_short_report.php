...
<html lang="en" dir="ltr">
  <head>
    <meta charset="UTF-8">
    <title>Customer Short Report</title>
...
<body style="background: #32aba9">
    ...
    <ul class="nav-links">
      ...
      <li>
        ...
        <ul class="sub-menu">
          ...
          <li><a href="#">10. Customer Short Report</a></li>
  ...
  <section class="home-section" style="background: #32aba9;">
    ...
    <div class="container col-lg-10 rounded-3 text-center" style="background: white;">
      ...
      <div id="PagePrintDiv" class="row overflow-scroll">
        <table id="table" class="table table-striped table-bordered">
          ...
          <div style="width:100%" class="d-flex col-lg-12 col-md-12 col-sm-12 text-center">
            ...
            <h5 class="headline2 col-lg-4 dtoSize">Operator : <?php echo $username; ?></h5>
          </div>
        	<h2 class="headline2">Customer Short Report</h2>
          <thead>
	          <tr>
	            <th>SL</th>
          ...
          <tbody>
            ...
            $count=1;
            $globalAmount=0;
            $subQuery="SELECT `customer_name`,`customer_address`,`customer_mobile`,`old_amount` FROM `customer_setup`";
            $result=mysqli_query($con,$subQuery);
              while($subRow=mysqli_fetch_assoc($result)){
                $saleAmount=0;
                $colAmount=0;
                $totalAmount=0;
                $oldCusAddress=$subRow['customer_address'];
                $oldCusName=$subRow['customer_name'];
                $oldCusMobile=$subRow['customer_mobile'];
                $oldAmount =$subRow['old_amount'];
              $query="SELECT SUM(`amount`) AS amount FROM `parts_sales`
                      WHERE `sales_by`='Due' AND `customer_address`='$oldCusAddress' AND `customer_name`='$oldCusName'";
              $q1=mysqli_query($con, $query);
              if(mysqli_num_rows($q1) > 0){
                while($row1=mysqli_fetch_assoc($q1)){
                  $saleAmount=$row1['amount'];
                  $totalAmount += $saleAmount;
                }
              }
              $query2="SELECT SUM(`amount`) AS amount2 FROM `customer_collection`
                      WHERE `customer_address`='$oldCusAddress' AND `customer_name`='$oldCusName'";
              $q2=mysqli_query($con, $query2);
              if(mysqli_num_rows($q2) > 0){
                while($row2=mysqli_fetch_assoc($q2)){
                  $colAmount=$row2['amount2'];
              ...
              if($totalAmount > 0){
                  ?>
                    ...
                    <tr>
                      ...
                      <td><?php echo $oldCusAddress; ?></td>
                      <td><?php echo $oldCusName; ?></td>
                      <td><?php echo $oldCusMobile; ?></td>
                ...
                ?>
                    <tr>
                      <td style="color: red;font-weight: bold;" colspan="4">Total :</td>
  ...
  <script>
          ...
          $('#printBtn').click(function(){
            ...
            var myWindow = window.open('customer_print_report.php', 'Customer Short Report',
