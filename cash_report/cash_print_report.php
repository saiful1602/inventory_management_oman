<?php
include("../DBconfig.php");
$id=$_SESSION['id'];
$Query="SELECT *FROM `accounts` WHERE `id`='$id'";
$result=mysqli_query($con, $Query);
$row=mysqli_fetch_assoc($result);
$user=$row['id'];
$username=$row['first_name'];
if(!isset($_SESSION['id'])){
  header('Location: ../Account/Login.php');
}


?>



<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <meta charset="UTF-8">
    <title>Cash Report</title>
    <link rel="stylesheet" href="../style.css">
    <!-- Boxiocns CDN Link -->
    <link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/2.1.2/sweetalert.min.js"></script>
    <script src = "https://code.jquery.com/jquery-3.4.1.js"></script>
    <script src="https://code.jquery.com/ui/1.13.2/jquery-ui.js"></script>
    <script type="text/javascript" src="https://momentjs.com/downloads/moment-with-locales.js"></script>
    <link rel="stylesheet" href="//code.jquery.com/ui/1.13.2/themes/base/jquery-ui.css">
    <style type="text/css">
      label{
        color: white;
      }
      .headline{
        text-shadow: .5px .5px #000000;
        color: #fff;
      }
      .headline2{
        text-shadow: .5px .5px #000000;
        color: red;
      }
      .headline3{
        text-shadow: .5px .5px #000000;
        color: black;
      }
      .container::-webkit-scrollbar { 
          display: none;  
      }
      .swal2-popup {
        font-size: 3.6rem !important;
      }
      .divScroll::-webkit-scrollbar {
        display: none;
      }
      body{
        font-size: 55%;
      }
      .table th, .table td{
        padding: 1.5px 7px 1.5px 7px; /* Apply cell padding */
      }
      .table th{
        font-size: 11px;
      }
      #table th{
        color: green;
      }
      #table2 th{
        color: red;
      }
      .tborder{
        border: 1px solid black;
      }

      @media print
      {    
          #cashDiv{
            display: none;
            visibility: none;
          }
          body{
            font-size: 55%;
          }
          .table th, .table td{
            padding: 1.5px 3px 1.5px 3px; /* Apply cell padding */
          }
      }





    </style>
     <meta name="viewport" content="width=device-width, initial-scale=1.0">
   </head>
  <body>
    
    <div class="container col-lg-12 rounded-3 text-center" style="background: white;">
      <h5 style="color:#130091;margin: 0;" class="headline2">علي بن محمد بخيت بيت مبارك للتجارة</h5>
      <h4 style="color:#130091;margin: 0;" class="headline2">ALI BIN MOHAMMED BAKHAIT BAIT MOBARAK TRAD . EST</h4>
      <h4 style="color:#130091;margin: 0;" class="headline2 mb-3">Sahalnoot Saniya , Salalah, Sultanate of Oman</h4>
      <div class="d-flex col-lg-12 col-md-12 col-sm-12 text-center">
        <h6 style="width:33%;text-align: center;" id="curDate" class="headline2 col-lg-4 col-md-4 col-sm-4 text-center">Print Date : </h6>
        <h6 style="width:33%;text-align: center;" id="curTime" class="headline2 col-lg-4 col-md-4 col-sm-4 text-center">Print time : </h6> 
        <h6 id="getDate1" class="headline2 col-lg-4" hidden><?php echo $_GET['date1']; ?></h6>
        <h6 style="width:33%;text-align: center;" class="headline2 col-lg-12 col-md-12 col-sm-12 text-center">Operator : <?php echo $username; ?></h6>
      </div>
      <h4 class="headline3">Daily Cash Report (SAYED) </h4>
      <div style="width:100%;" class="d-flex col-lg-12 col-md-12 col-sm-12 mx-auto text-center">
        <h6 style="width:33%;text-align: center;color: #130091;" id="slMonth" class="headline2 col-lg-4 col-md-4 col-sm-4">Month : </h6>
        <div style="width:34%;" class="col-lg-8 col-md-8 col-sm-8"></div>
        <h6 style="width:33%;text-align: center;color: #130091;" id="slDate" class="headline2 col-lg-4 col-md-4 col-sm-4">Date : </h6>
      </div>
      
    
  </div>

  <div style="padding:0 20px 0 20px;" class="divScroll col-lg-12 text-center d-flex overflow-scroll" >

      <!-- 1st Table -->

      <div class=" col-lg-7">
        <table id="table" class="table table-striped table-bordered">
        </table>
      </div>

      <!-- Second Table -->

      <div class=" col-lg-5">
        <table id="table2" class="table table-striped table-bordered">
        </table>
      </div>

    </div>

   <div style="background: #fff;padding-top: 30px;" class="container col-lg-12 rounded-3 text-center d-flex overflow-scroll mb-3">
      <div class=" col-lg-4 text-center mx-auto d-flex">
        <div class="col-lg-8">
          <h5 id="HLine" class="headline2">Net Cash report</h5>
          <table id="table3" class="table table-striped table-bordered">
            
          </table>
          <div style="padding: 20px 0 40px 0;" id="cashDiv">          
            <button style="margin-left: 20px;" id="netCashBtn" class="btn btn-success">Cash Transfer</button>
          </div>
        </div>
        <div class="col-lg-4">
          <div style="padding: 2rem 0 0 2rem;">
            <table id="table4" class="table table-striped table-bordered">
              
            </table>
          </div>
        </div>
        
        
      </div>
      

      <div align="left" class="col-lg-4">
      </div>
    </div> 

  
  <script>


    var d = new Date();

    var cuDateFormat=d.getDate() + "-" + (d.getMonth()+1) + "-" + d.getFullYear();

    var datetime = new Date();
    datetime = moment(datetime).format("YYYY-MM-DD");

    let today = new Date().toLocaleDateString();

    $(document).ready(function(){
      $('#curDate').text("Print Date : "+cuDateFormat)
    })

    var time = new Date();
    $('#curTime').text("Print Time : " + time.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }));

	  $(document).ready(function(){
	  	var date1 = $('#getDate1').text();

      if(date1 < datetime){
        $('#cashDiv').show();
      }else{
        $('#cashDiv').hide();
      }


      if(date1 != ""){
	  		$.ajax({
					url : "crSelect.php",
					type : "GET",
					data: { 
						'date1' : date1
					},
					success : function(data){
						$('#table').html(data);
						
					}
				});
        $.ajax({
          url : "crSelect2.php",
          type : "GET",
          data: { 
            'date1' : date1
          },
          success : function(data){
            $('#table2').html(data);
            
          }
        });
        $.ajax({
          url : "crSelect3.php",
          type : "GET",
          data: { 
            'date1' : date1
          },
          success : function(data){
            $('#table3').html(data);
            
          }
        });
        $.ajax({
          url : "crSelect4.php",
          type : "GET",
          data: { 
            'date1' : date1
          },
          success : function(data){
            $('#table4').html(data);
            
          }
        });

        
        $('#HLine').show();

       

        const searchDate = new Date(date1);  // 2009-11-10
        const month = searchDate.toLocaleString('default', { month: 'long' }) + "/" + searchDate.getFullYear();
        const Sdate = searchDate.getDate() + "-" + (searchDate.getMonth()+1) + "-" + searchDate.getFullYear();
        $('#slMonth').append(month);
        $('#slDate').append(Sdate);


      }else{
        swal("Please Select Valid Date")
      }
			
	  });

    $('#netCashBtn').click(function(){
      $('#netCashBtn').attr("disabled", "disabled");

      var date1 = $('#getDate1').text();
      swal({
          title: "Are you sure?",
          text: "Transfer Cash!!",
          icon: "warning",
          buttons: [
            'No, cancel it!',
            'Yes, I am sure!'
          ],
          dangerMode: true,
        }).then(function(isConfirm) {
          if (isConfirm) {
            swal({
              title: 'Message!',
              text: 'Cash has been transferred successful',
              icon: 'success'
            }).then(function(){

              $.ajax({
              url : "crSelect5.php",
              type : "POST",
              data: { 
                'date1' : date1
              },
              success : function(data){
                
              }
            });
         });
          } else {
            swal("Cancelled", "Please check data :)", "error");
          }
        });
      });  


      /************ Update logger info **************/

      function updateUserStatus(){
        jQuery.ajax({
          url:'update_user_stats.php',
          success:function(){
            
          }
        });
      }


      setInterval(function(){
        updateUserStatus();
      },3000);	  
	  

	</script>
  

</body>
</html>
