...
include('../DBconfig.php');
  ...
  if(mysqli_num_rows($q7) > 0){
   ...
   $query="SELECT `date` FROM `cash_transfer` WHERE `date`='$today'";
   $que=mysqli_query($con, $query);

   if(mysqli_num_rows($que) > 0){
       $query3="UPDATE `cash_transfer` SET `amount`='$netTotal',`refDate`='$date1' WHERE `date`='$today'";
       mysqli_query($con, $query3);
       
   }else{
       $query2="INSERT INTO `cash_transfer`(`amount`,`date`,`refDate`)VALUES('$netTotal','$today','$date1')"; 
